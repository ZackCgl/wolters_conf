          ...
          if (xmlhttp.readyState == 4) {
            if (xmlhttp.status == 200) {
              ...
              for(let i= 0; i < 500; i++){
                ...
                suppArray.push(demension?.getElementsByTagName("code")[0]?.textContent)
            ...
            if (xmlhttp.status == 200) {
             ...
             const xmlDoc2 = parseHtml.parseFromString(firstRelaties,"text/xml");
              ...
              for(let i = 0; i < 100; i++) {
               ...
               supArrayPro.push(line?.getElementsByTagName("name")[0]?.textContent)
              }
              ...
              for(let i = 0; i < 100; i++) {
               ...
               supArrayUn.push(line?.getElementsByTagName("units")[0]?.textContent)
              }
              setUnits(supArrayUn)
              ...
              for(let i = 0; i < 100; i++) {
               ...
               supArrayUnitspriceexcl.push(line?.getElementsByTagName("unitspriceexcl")[0]?.textContent)
              }
              ...
              for(let i = 0; i < 100; i++) {
               ...
               supArrayUnitspriceinc.push(line?.getElementsByTagName("unitspriceinc")[0]?.textContent)
              }
              setInc(supArrayUnitspriceinc)
               ...
               for(let i = 0; i < 100; i++) {
                ...
                supArrayGrootboek.push(line?.getElementsByTagName("freetext1")[0]?.textContent)
               }
            ...
            <div ref={parentPage} className="flex ml-8 mt-20">
              ...
              <div className='text-white'>
                ...
                <div ref={parentDebiteuren}>
                  <button onClick={selectCustomers} className='dropdown-label mt-4 flex flex-col rounded-xl bg-white/10 p-1.5
                  text-white hover:bg-white/20'>Selecteer Relatie</button>
                  {show && <div className=" h-40 overflow-y-scroll scrollbar-hide text-white font-extralight text-1xl">{relations?.map((sup:any, i:any) => {
                    return <div className='' key={i}><p onClick={() => {setChosenCustomer(sup); setShow(false)}} className='dropdown-content hover:font-semibold cursor-pointer'>{sup}</p></div>
                  })}
                  </div>}
                </div>
                <div>
                 <Link href={`/relaties/toevoegen#id_token=${fullsplit}`}><button className='ml-2 dropdown-label mt-4 flex flex-col rounded-xl bg-white/10 p-1.5
                ...
                <div className='flex'>
                <div ref={parent}>
                <button onClick={selectProducts} className='dropdown-label mt-4 flex flex-col rounded-xl bg-white/10 p-1.5
                 text-white hover:bg-white/20'>Selecteer product</button>
                  {isClickedProductsButton && <div className='flex items-center'>
                    <input placeholder='"12345"' className='dropdown-content rounded-md mt-2 h-7 ml-2 w-16 text-black placeholder-slate-500' value={products} onChange={(e) => setProducts(e.target.value)} />
                    <button onClick={searchProducts} className='ml-2 mt-2 flex flex-col rounded-xl bg-white/10 p-1.5
                 text-white hover:bg-white/20'>Zoek</button>
                 </div>}
                 </div>
                 
                 <div>
                 <Link href={`/producten/aanmaken#id_token=${fullsplit}`}><button className='ml-2 dropdown-label mt-4 flex flex-col rounded-xl bg-white/10 p-1.5
                 text-white hover:bg-white/20'>Product toevoegen</button></Link>
                 </div>
                 </div>
                    <div className='flex'>
                      <div className='p-2'>
                      <label>Code</label><p>{soapProducts}</p>
                      </div>
                      <div className='p-2'>
                      <label>Naam</label><p>{soapName}</p>
                      </div>
                      <div className='p-2'>
                      <label>Producten</label><p>{producten?.map((line:any, i:number) => {
                        return <div key={i}>{line}</div>
                      })}</p>
                      </div>
                      <div className='p-2'>
                      <label>Eenheden</label><p>{units?.map((unit:any, i:number) => {
                        return <div key={i}>{unit}</div>
                      })}</p>
                      </div>
                      <div className='p-2'>
                      <label>Vatcode</label><p>{vatcode}</p>
                      </div>
                      <div className='p-2'>
                      <label>Ex. btw</label><p>{exbtw?.map((ex:any, i:number) => {
                        return <div key={i}>{ex}</div>
                      })}</p>
                      </div>
                      <div className='p-2'>
                      <label>Inc. btw</label><p>{incbtw?.map((inc:any, i:number) => {
                        return <div key={i}>{inc}</div>
                      })}</p>
                      </div>
                      <div className='p-2'>
                      <label>Grootboek</label><p>{grootboek?.map((grootboek:any, i:number) => {
                        return <div key={i}>{grootboek}</div>
                      })}</p>
                      </div>
                      <div className='p-2'>
                      <label>Totaal</label><p>TBC</p>
                      </div>
                      
                    </div>
                 
                <button onClick={addInvoices} className='mt-4 flex flex-col rounded-xl bg-purple-800 p-2
                 text-white hover:bg-purple-900'>Create Invoice</button>
                </div>
