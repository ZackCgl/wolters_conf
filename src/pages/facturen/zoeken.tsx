    ...
    xmlhttp.onreadystatechange = () => {
      if (xmlhttp.readyState == 4) {
        if (xmlhttp.status == 200) {
          ...
          const suppliers: any =
            xmlDoc2.getElementsByTagName("salesinvoice")[0];
          ...
          if (demension == undefined) {
            ...
            const invoicenumber: any =
              requestedInvoiceNumber > 0 &&
              demension.getElementsByTagName("invoicenumber")[0];
            const customer: any =
              requestedInvoiceNumber > 0 &&
              demension.getElementsByTagName("customer")[0];
            const valueinc: any =
              requestedInvoiceNumber > 0 &&
              totals.getElementsByTagName("valueinc")[0];
            const valueex: any =
              requestedInvoiceNumber > 0 &&
              totals.getElementsByTagName("valueexcl")[0];
          ...
          <div className="ml-8 mt-20 flex">
            ...
            {accesToken && (
              <div className="text-white">
                <p className=" flex-col text-3xl font-bold">Zoeken</p>
                <div>
                  <label>Zoek factuurnummer: </label>
                  <input
                    className="w-16 rounded-md text-black"
                    placeholder="bijv: 1"
                    value={requestedInvoiceNumber}
                    onChange={(e) => setRequestedInvoiceNumber(e.target.value)}
                  />
                  <button
                    className="mt-4 flex flex-col rounded-xl bg-white/10 p-2 text-white hover:bg-white/20"
                    onClick={getInvoices}
                  >
                    Zoek factuur
                  </button>
